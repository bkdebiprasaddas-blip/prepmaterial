# AGENTS.md — Project Rule Book (PrepMaterial)

> Full governance source: `governance\RULEBOOK.md` (PART I + PART II).
> Sections §A, §D, §F, §H, §J below are copied verbatim from that file
> per §I.5. This file is the auto-discovered entry point; keep it short —
> everything else (specs, logs, plans) lives under `governance\`.
...
## Project-specific section (PrepMaterial)

- **Structure:** ROOT = production code (none yet) + `AGENTS.md` + `governance\`.
  `governance\` = continuity (tracked in git once initialized). `Scratch\<ProjectName>\`
  = disposable prep material (git-ignored) — not yet created; project name pending.
- **Current gate:** PRE-DISCOVERY. No product definition exists yet — this
  repository contained only `RULEBOOK.md` when the scaffold was built.
  See `governance\BOOTSTRAP.md` for the live snapshot.
- **Quick links:** `governance\RULEBOOK.md` (full rules), `governance\BOOTSTRAP.md`
  (current state), `governance\ai-context\` (session logs), `governance\work-log\`
  (plain-English logs), `governance\planning\` (specs, empty until DISCOVERY/PLANNING).
- **Rules of the house:** nothing is approved yet; no stack, git, or hosting
  decision has been made. Do not assume any of them.
